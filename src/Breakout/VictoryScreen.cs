...
internal sealed class VictoryScreen(BreakoutGameState state, IScreenCoordinator coordinator) : GameScreen
{
    public override void Draw(SKCanvas canvas, int width, int height)
    {
        ...
        TextRenderer.DrawCenteredText(canvas, "YOU WIN!", 64f, new SKColor(0xFF, 0xD6, 0x0A), GameWidth / 2f, 270f);
        TextRenderer.DrawCenteredText(canvas, $"Final Score: {state.Score}", 32f, SKColors.White, GameWidth / 2f, 335f);
